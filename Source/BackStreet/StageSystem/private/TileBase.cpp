...
void ATileBase::SpawnItem()
{
	...
	for (int8 i = 0; i < spawnMax; i++)
	{
		...
		FActorSpawnParameters actorSpawnParameters;
		actorSpawnParameters.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AdjustIfPossibleButAlwaysSpawn;
		AItemBase* target = GetWorld()->SpawnActor<AItemBase>(InGameScriptRef->GetAssetManager()->ItemAssets[type], ItemSpawnPoints[i]->GetActorLocation(), FRotator::ZeroRotator, actorSpawnParameters);
		if (type == 6)
		{
...
void ATileBase::SpawnMission()
{
	...
	if(Mission->Type == 1) // 아이템 습득
	{
		FActorSpawnParameters actorSpawnParameters;
		actorSpawnParameters.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AdjustIfPossibleButAlwaysSpawn;
		AItemBase* target = GetWorld()->SpawnActor<AItemBase>(InGameScriptRef->GetAssetManager()->MissionAssets[0], MissionSpawnPoints[0]->GetActorLocation(), FRotator::ZeroRotator, actorSpawnParameters);
